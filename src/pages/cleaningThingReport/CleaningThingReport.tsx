...
const CleaningThingReport = () => {
  const cleaningThingId = new URLSearchParams(window.location.search).get(
    'cleaningThingId',
  )
  ...
  useEffect(() => {
    ...
    getCurrentCount(cleaningThingId).then((count) => {
      setCurrentCount(count)
      setIsCurrentCountLoading(false)
    })

    getCleaningThingHistory(cleaningThingId).then((data) => {
      setReportData(data)
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-2xl font-semibold text-white">
                {getItemName(cleaningThingDetail, cleaningThingId)} Overview
              </h1>
            </div>
          </div>
